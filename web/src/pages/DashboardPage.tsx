    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-3">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <h1 className="text-2xl font-display font-bold tracking-tight">Momentum Dashboard</h1>
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            >
              ...
              Filters
              {activeFilterCount > 0 && (
                <span className="bg-accent text-white text-xs w-4 h-4 rounded-full flex items-center justify-center">
                  {activeFilterCount}
                </span>
              )}
            </button>
      ...
      <div className="card overflow-hidden">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-surface-overlay text-steel text-[10px] uppercase tracking-widest border-b-2 border-accent/20">
          ...
          <tbody>
            {loading ? (
              <tr>
                <td
                  colSpan={10}
                  className="px-4 py-12 text-center text-steel"
                >
              ...
              allArtists.map((a, idx) => (
                <tr
                  key={a.spotify_id}
                  className="border-b border-surface-border/40 hover:bg-surface-overlay/80 transition-all group animate-slide-up accent-border-hover"
                  style={{ "--delay": `${Math.min(idx, 15) * 30}ms` } as React.CSSProperties}
                >
                  ...
                  <td className="px-3 py-3">
                    <Link
                      ...
                      className="flex items-center gap-2.5 font-medium text-gray-200 hover:text-white transition-colors"
                    >
                      {a.image_url ? (
                        <img
                          src={a.image_url}
                      ...
                      <span className="group-hover:translate-x-0.5 transition-transform">{a.name}</span>
                    </Link>
                  </td>
                  ...
                  <td className="px-3 py-3 text-right font-mono text-steel hidden md:table-cell">
                    {a.trajectory?.toFixed(0) ?? "--"}
                  </td>
                  <td className="px-3 py-3 text-right font-mono text-steel hidden md:table-cell">
                    {a.industry_signal?.toFixed(0) ?? "--"}
                  </td>
                  <td className="px-3 py-3 text-right font-mono text-steel hidden lg:table-cell">
                    {a.engagement?.toFixed(0) ?? "--"}
                  </td>
                  <td className="px-3 py-3 text-right font-mono text-steel hidden lg:table-cell">
                    {a.release_positioning?.toFixed(0) ?? "--"}
                  </td>
                  <td className="px-3 py-3 text-steel text-xs hidden xl:table-cell max-w-[150px] truncate">
